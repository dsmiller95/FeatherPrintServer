...
export class DatabasePopulator{
	...
	private requestBodyForBird(birdName: string) : Promise<any> {
	...
	public buildQuery(model: BirdModel): string{
		var query = "INSERT INTO `information`(`birdName`,`description`,`scientificName`,`behavior`,`conservation`)";
		query = query + ` Values ("${model.name}", "${model.desc}", "${model.sciName}", "${model.behavior}", "${model.status}")`
		return query;
	}
	...
	private parse(body: string): void{
		...
		connection.query(query, function(err, result) {
			if (err){
				console.log("Failed to add information entry: " + err);
			}else{
		...
		//Main image
		this.imageUrl(model.imageUrl, model);
		
		//Range
		this.rangeUrl(model.rangeUrl, model);
	...
	private parseConservationStatus(raw: string): string{
	...
	private parseBehavior(raw: string): string{
	...
	public doThePopulate(){
		...
		this.requestBodyForBird(birdName).then((body) => {
			this.parse(body);
		});
